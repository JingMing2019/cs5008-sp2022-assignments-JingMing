...
int main () {
  ...
  // initialize `done` to false - not done with any node yet
  for (i = 0; i < GSIZE; i++) {
    done[i] = false;
  }
  ...
  while (!isEmpty(q)) {
    ...
    current = dequeue(q);
    if(!done[current]) { // if node is not done
      printf("Node: %d\n", current);
      for(j = 0; j < GSIZE; j++) {
        if(E[current][j]) { // if there exists a path
            enqueue(q, j);
        }
      }
      done[current] = true; // after push all the child into the queue, mark this node as done
    }
  }
