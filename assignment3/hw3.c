...
bool isEmpty(queue_t* q_p) {
  ...
  // ***** INSERT YOUR CODE HERE *****
  if (q_p != NULL) { // have to check wether pointer is NULL or not
    b = (q_p -> head_p == NULL);

    if (b && (q_p -> tail_p != NULL)) { // have to chech head and tail both NULL
