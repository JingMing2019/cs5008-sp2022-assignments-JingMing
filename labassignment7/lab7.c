...
/* Hospital emergency room system using priority queue */
...
node_t *insert(struct node *front)
{
    ...
    //The queue is empty or priority is larger than the front's priority
    if (front == NULL || pri > front -> priority) {
        temp -> next = front;
        front = temp;
        return front;
    }
    //Otherwise, traverse the list and find the correct position.
    //If patients with same priority, first in first out.
    p = front;
